...
import { getRulesForGame, legalMoves } from '../engine'
...
import { evaluateStrong, evaluateExpert, WIN_SCORE, MAX_PLY, WEIGHTS_BY_GAME } from './evaluation'
...
export class WorkerMessageHandler {
  ...
  private tryLoadTablebase(game: GameId): void {
    if (this.tbReady || this.tbLoadInFlight) return
    this.tbLoadInFlight = true
    loadTablebaseFromIDB(game)
      .then((cached) => {
  ...
  handleMessage(msg: BotMessage): void {
    ...
    if (msg.type === 'pickMove') {
      ...
      if (msg.level === 'expert' && (!this.tbReady || this.tbGame !== game)) {
        this.tbReady = false
        this.tryLoadTablebase(game)
      }
  ...
  private handlePickMove(msg: BotRequest): void {
    ...
    const { state, level, timeBudgetMs, requestId, game } = msg
    const rules = getRulesForGame(game ?? 'kalah')
    ...
    try {
      ...
      this.runAsyncSearch(state, level, timeBudgetMs, rules, requestId, cancelSignal, game ?? 'kalah')
    } catch (err) {
      this.postMsg({
        type: 'error',
        requestId,
  ...
  private runAsyncSearch(
    ...
    const budget = timeBudgetMs ?? (level === 'strong' ? 1500 : 3500)
    const deadlineMs = startTime + budget
    const weights = WEIGHTS_BY_GAME[game]
    const evalFn = level === 'strong'
      ? evaluateStrong
      : (s: GameState, r: RuleConfig) => evaluateExpert(s, r, weights)
